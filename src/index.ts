...
const simulateMigrationProgress = async (
  ...
  setTimeout(async () => {
    try {
      ...
      if (result.success) {
        console.log(
          `🎉 Migration ${migrationId} completed with ${
...
app.post(
  ...
  (
    req: ExtendedRequest,
    res: Response<StartMigrationResponse | { error: string }>
  ) => {
    ...
    const environmentId =
      req.headers["x-contentful-environment-id"] || "master";

    if (!spaceId) {
      return res
        .status(400)
        .json({
          error: "Space ID is required in X-Contentful-Space-Id header",
        });
    }
...
app.get(
  ...
  (
    req: ExtendedRequest,
    res: Response<MigrationStatusResponse | { error: string }>
  ) => {
